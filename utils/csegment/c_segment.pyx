...
the C funtion: c_run_segmentation post-process class and offset prediction
"""
...
cdef extern void c_run_segmentation(float * class_mask, int class_dim,
                                    float * bound_mask, int offset_dim,
...
def run_segmentation(np.ndarray[float, ndim=3, mode="c"] class_pred not None,
    ...
    e. mask_pred is a (img_height, img_width) matrix. It is used to store result.
    ...
    cdef np.ndarray mask_pred = np.zeros((img_height, img_width)).astype(np.int32)
    cdef np.ndarray object_class_pred = np.zeros((1, img_height * img_width)).astype(np.int32)
