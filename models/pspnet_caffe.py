""" This script is adpated from 
    https://github.com/meetshah1995/pytorch-semseg/blob/master/ptsemseg/models/pspnet.py
    It is used to convert the released pspnet caffe model to pytorch model. 
"""
...
class bottleNeckPSP(nn.Module):
    ...
    def __init__(self, in_channels, mid_channels, out_channels,
        ...
        self.cbr1 = conv2DBatchNormRelu(
            in_channels, mid_channels, 1, stride=1, padding=0, bias=bias, with_bn=with_bn)
        if dilation > 1:
        ...
        self.cb3 = conv2DBatchNorm(
            mid_channels, out_channels, 1, stride=1, padding=0, bias=bias, with_bn=with_bn)
        self.cb4 = conv2DBatchNorm(
            in_channels, out_channels, 1, stride=stride, padding=0, bias=bias, with_bn=with_bn)
    ...
    def __init__(self, in_channels, mid_channels, stride, dilation=1, with_bn=True):
        ...
        if dilation > 1:
            ...
            self.cbr2 = conv2DBatchNormRelu(mid_channels, mid_channels, 3,
                                            ...
                                            bias=bias, dilation=1, with_bn=with_bn)
        self.cb3 = conv2DBatchNorm(
            mid_channels, in_channels, 1, stride=1, padding=0, bias=bias, with_bn=with_bn)
...
class residualBlockPSP(nn.Module):
    ...
    def __init__(self, n_blocks, in_channels, mid_channels, out_channels, stride, dilation=1, include_range='all', with_bn=True):
        ...
        if include_range in ['all', 'conv']:
            layers.append(bottleNeckPSP(in_channels, mid_channels,
                                        out_channels, stride, dilation, with_bn=with_bn))
        if include_range in ['all', 'identity']:
            for i in range(n_blocks - 1):
                layers.append(bottleNeckIdentifyPSP(
...
def multi_scale_cross_entropy2d(input, target,
                                weight=None, size_average=True, scale_weight=None):
...
class pspnet(nn.Module):
    ...
    def __init__(self,
        ...
        self.res_block2 = residualBlockPSP(
            self.block_config[0], 128, 64, 256, 1, 1)
        self.res_block3 = residualBlockPSP(
            self.block_config[1], 256, 128, 512, 2, 1)

        # Dilated Residual Blocks
        self.res_block4 = residualBlockPSP(
            self.block_config[2], 512, 256, 1024, 1, 2)
        self.res_block5 = residualBlockPSP(
            self.block_config[3], 1024, 512, 2048, 1, 4)
    ...
    def load_pretrained_model(self, model_path):
        ...
        def _transfer_conv(layer_name, module):
            ...
            module.weight.data.copy_(torch.from_numpy(
                weights).view_as(module.weight))
        ...
        def _transfer_conv_bn(conv_layer_name, mother_module):
            ...
            if self.verbose:
                print("BN {}: Original {} and trans weights {}".format(conv_layer_name,
                                                                       ...
                                                                       mean.shape))
            bn_module.running_mean.copy_(torch.from_numpy(
                mean).view_as(bn_module.running_mean))
            bn_module.running_var.copy_(torch.from_numpy(
                var).view_as(bn_module.running_var))
    ...
    def tile_predict(self, imgs, n_instance_classes, include_flip_mode=True):
        """
        ...
        n_samples, color, h, w = imgs.shape
        ...
        pred = torch.zeros(n_samples, n_instance_classes, h, w)
        ...
        for sx, ex in x_ends:
            for sy, ey in y_ends:
                slice_count += 1
                # print('slice: {}'.format(slice_count))
                imgs_slice = imgs[:, :, sx:ex, sy:ey]

                if include_flip_mode:
                    imgs_slice_flip = torch.from_numpy(
                        np.copy(imgs_slice.cpu().numpy()[:, :, :, ::-1])).float()

                is_model_on_cuda = next(self.parameters()).is_cuda

                if is_model_on_cuda:
                    imgs_slice = imgs_slice.cuda()
                    if include_flip_mode:
                        imgs_slice_flip = imgs_slice_flip.cuda()

                with torch.no_grad():
                    psub1 = F.softmax(self.forward(imgs_slice),
                                      dim=1).cpu().numpy()
                    if include_flip_mode:
                        psub2 = F.softmax(self.forward(
                            imgs_slice_flip), dim=1).cpu().numpy()
                    ...
                    psub_instance[:, 1:, :, :] = psub[:,
                                                      n_classes - n_instance_classes + 1:, :, :]
                    pred[:, :, sx:ex, sy:ey] += psub_instance
                    count[sx:ex, sy:ey] += 1.0

        score = (pred / count[None, None, ...]).float()
