...
import os
import random
from models import UNet, FCNResnet, FCNVGG16
import torch.optim.lr_scheduler as lr_scheduler
from dataset import COCODataset
from loss import CrossEntropyLossOneHot, SoftDiceLoss, MultiBCEWithLogitsLoss
from waldo.core_config import CoreConfig
from waldo.train_utils import train, validate, sample, save_checkpoint, soft_dice_loss, runningScore
...
def main():
    ...
    trainset = COCODataset(args.train_img, args.train_ann, c_config,
                           ...
                           class_nms=class_nms, limits=200, cache=True)
    trainloader = torch.utils.data.DataLoader(
        trainset, num_workers=2, batch_size=args.batch_size, shuffle=True)
    valset = COCODataset(args.val_img, args.val_ann, c_config,
                         (args.train_image_size, args.train_image_size * 2),
                         class_nms=class_nms, limits=100, cache=True)
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        iterations = train(trainloader, model, criterion_cls, criterion_ofs,
                           ...
                           alpha=args.alpha)
        val_loss = validate(valloader, model, criterion_cls, criterion_ofs,
